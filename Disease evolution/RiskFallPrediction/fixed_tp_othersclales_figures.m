...
FOCUS = ["Delta_MFISTOT" "Delta_FIM_TOT" "Delta_LIFETOT"];
names = ["negligible" "severe"];  
risk_class = cell(numel(FOCUS),1);
...
for i=1:numel(FOCUS)
    risk_class{i}(id_lowbase,1)=(discretize(Ttrain.(FOCUS(i))(id_lowbase),[-Inf L1 Inf],names)); % basso edss
    risk_class{i}(id_medbase,1)=discretize(Ttrain.(FOCUS(i))(id_medbase),[-Inf L2 Inf],names);   % medio edss
    risk_class{i}(id_highbase,1)=discretize(Ttrain.(FOCUS(i))(id_highbase),[-Inf L3 Inf],names); % alto edss

    risk_class{i} = categorical((risk_class{i}),names,'ordinal',true);
end
% Ttrain=addvars(Ttrain,risk_class);
temp_risk=[risk_class{1},risk_class{2},risk_class{3}];
risk_names= ["risk_classMFIS" "risk_classFIM" "risk_classLIFE"];
risk = array2table(temp_risk);risk.Properties.VariableNames=risk_names;
...
Ttrain.impairment=categorical(impairment.sum,0:4,{'none' 'low' 'medium' 'high' 'severe'},'ordinal',true);
dummy= rand(height(Ttrain),1);  % aggiunta dummy var 
base_risk = array2table(NaN(height(Ttrain),3));
base_names = ["base_riskMFIS" "base_riskFIM" "base_riskLIFE"];
base_risk.Properties.VariableNames=base_names;

Ttrain=[Ttrain,base_risk];
TtrainRF = addvars(Ttrain, cat_label ,dummy);
TtrainRF=[TtrainRF,risk];
...
for i=1:numel(pat)
    ...
    if Tpat.deltaT(1) <= t_range && Tpat.RELAPS(1) == 0 % delta T conta il tempo che apssa dopo il primo anno di fup (o tstar di fup)
       %base_risk(w,j)=risk_pat(1);
       TtrainRF(w,ismember(TtrainRF.Properties.VariableNames,base_names))=array2table(repmat(table2array(Tpat(1,risk_names)),nnz(w),1));
        
    end
    
end
TtrainRF(isnan(TtrainRF.(base_names(1))),:)=[];

% edss 443 pat and 2815 clinical records
%% predizione a 1y
%target_time=[2, 3, 4]; % visita piu vicina a 2 anni a meno di 6 mesi 
target_time=4;
time_range=0.5; % scarto consentito dall'anno di visita = 6 mesi
clear TRFtime sub_risk_base;
TRFtime = cell(numel(FOCUS),1);
pat = unique(TtrainRF.Paziente);
...
%for k=1:numel(FOCUS)
    for j=1:numel(pat)	
        ...
        % if nnz(w)>0
        Tpat=TtrainRF(w,:);
	    [~,sel]=min(abs((Tpat.deltaT-target_time)));
	    % w=find(Tpat.deltaT<time_range,1,'first');
	    if abs(Tpat.deltaT(sel)-target_time)<time_range
...
for k=1:numel(FOCUS)
    ...
    [freq(k,:),names(k,:)]=histcounts(TRFtime{k}.(risk_names(k)));
end
...
% vars = [demo proof base slope];
leaf = 1;
 %cat_morbius.Properties.VariableNames
Ttime = cell(numel(FOCUS),1);OOB = cell(numel(FOCUS),1);scores = cell(numel(FOCUS),1);predictions=cell(1,numel(target_time));

for k=1:numel(FOCUS)
    vars=[demo independent{:} base_names(k) 'eqCOUR' 'impairment' proof];
    out = risk_names(k);
    names = unique(TRFtime{k}.(risk_names(k)));
    ...
    parfor j=1:cv.NumTestSets
	    ...
	    fitmodel = fitcensemble(Ttime{k}(training(cv,j),:),out,...
    ...
    OOB{k}=oobvi;
    predictions{k}=Ttime{k}.(risk_names(k));
...
stats = cell(numel(FOCUS),1);
% if p<0.05
%     disp('keep going!');
% end
for k=1:numel(FOCUS)
    ax(k) = nexttile;
    [C,order]=confusionmat(Ttime{k}.(risk_names(k)),predictions{k},'order',names);confusionchart(C,order);
    accu(k)=sum(diag(C))/sum(C,'all');
    [~,~,p(k)]=crosstab(Ttime{k}.(risk_names(k)),predictions{k});
    title(sprintf('%s : Accuracy %1.1f %% (p value %1.3f) T=%d',FOCUS(k),accu(k)*100,p(k),target_time));
    ax = gca; 
    ax.FontSize = 12; 
    
   % ,'Interpreter','none'
    
    stats{k} = statsOfMeasure(C, 1);
end
...
SD_importance=cell(numel(FOCUS),1);mean_importance=cell(numel(FOCUS),1);
figure(4);clf;
ht=tiledlayout(1,numel(FOCUS))
for k=1:numel(FOCUS)
    vars=[demo independent{:} base_names(k) 'eqCOUR' 'impairment' proof];
    ...
    if k==1
        ...
        hold off
        title(ax(k),sprintf('%s: T = %d',FOCUS(k),target_time),'Interpreter','none')
        h = gca;
...
export_fig black_name_err -png -jpg -pdf -transparent
...
figure(5);clf;
%ht = tiledlayout('flow');
Tauc = cell(numel(FOCUS),3);
for k=1:numel(FOCUS)
    [Tauc{k,1}(:,1),Tauc{k,1}(:,2),Tauc{k,2},Tauc{k,3}] = perfcurve(Ttime{k}.(risk_names(k)),scores{k}(:,2),'severe'); %0.8234 PER t=3
end
% ax(2) = nexttile;
...
legend([h(1) h(2) h(3)],'MFIS','FIM','LIFE','Location','southeast')

ax = gca; 
ax.FontSize = 12; 

export_fig roc_all_BW -png -jpg -pdf -transparent
mat = [Tauc{1,3},size(Ttime{1},1),freq(1,:);Tauc{2,3}, size(Ttime{2},1),freq(2,:);Tauc{3,3}, size(Ttime{3},1),freq(3,:)];
mat = array2table(mat);
writetable(mat,'auc.xlsx') % ...size(Ttime{1},1);freq'],'auc.xlsx')

%% definizione di evento(i)
EDSSevent=cell(numel(FOCUS),1);
MODELevent=cell(numel(FOCUS));
for k=1:numel(FOCUS)
    % EDSSevent=TRFsub.classes=='3'; % RISK CLASS
    EDSSevent{k}=Ttime{k}.(risk_names(k))=='severe'; % RISK CLASS 
    MODELevent{k}=predictions{k}=='severe';
end
%% creazione tabella x SURVIVAL analysis
Tsurv = cell(numel(FOCUS),1);Tsurv_complete = cell(numel(FOCUS),1);
for k=1:numel(FOCUS)
...
%ht=tiledlayout("flow");
 ...
 for k=1:numel(FOCUS)
   ...
   % ax(k)=nexttile;
    ...
    title(sprintf('%s: Kaplan Meier curves (T=%d yrs)', FOCUS(k), target_time),'Interpreter','none')
    ax2.FontSize = 12;
